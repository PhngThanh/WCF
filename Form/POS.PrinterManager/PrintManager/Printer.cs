...
using POS.Repository.PrinterModel;
...
namespace PrinterManager.PrintManager
{
    public class Printer
    {
        ...
        private static StoreInfo storeInfo = PrinterManager.Configuration.Configuration.GetStoreInfo();
        private static PosConfig posConfig = PrinterManager.Configuration.Configuration.GetPosConfig();
        //private static List<ProductViewModel> AvailableSingleProducts= Configuration.Configuration.GetAvailableSingleProducts();
        //private static List<CategoryViewModel> AvailableCategories = Configuration.Configuration.GetAvailableCategories();
        //private static List<ProductViewModel> AllProducts = Configuration.Configuration.GetAllProducts();
        //private static List<CategoryExtraViewModel> AllCategoryExtras = Configuration.Configuration.GetAllCategoryExtras();
        //private static List<PromotionEditViewModel> Promotions = Configuration.Configuration.GetPromotions();
        //private static List<PromotionDetailViewModel> PromotionDetails = Configuration.Configuration.GetPromotionDetails();
        //private static List<TableViewModel> AllTables = Configuration.Configuration.GetAllTable();
        private static CultureInfo CultureInfo = Configuration.Configuration.GetCultureInfo(); //Declare culture info
        private static ResourceManager ResManager= Configuration.Configuration.GetResourceManager();
        ...
        public void CustomPrintLocation(PrintDocument printDocument, string name)
        {
            try
            {
                ...
                if (printDocument.PrinterSettings.PrinterName == "Microsoft Print to PDF")
                {
                    ...
                    var date = Utils.UtcDateTimeNow().ToString("dd");
                    var yearAndMonth = Utils.UtcDateTimeNow().ToString("yyyy-MMM");
        ...
        public void PrintBillDetail(OrderPrinterModel order, OrderDetailViewModel orderDetail,
                                    string printerName)
        {
        ...
        public void PrintBill(OrderPrinterModel order, BillTypeEnum billType,
                                    string printerName, bool isPrintAllOrderDetail, int? printGroup = null)
        {
            try
            {
                ...
                else if (storeInfo.PrinterDetail.ToLower() == "true".ToLower())
                {
                    var whitelistSplit = storeInfo.Whitelist.Split('|');
                    var whitelistCode = new List<int>();
                    foreach (var item in whitelistSplit)
                    {
                        whitelistCode.Add(int.Parse(item));
                    }
                    var ordersOfMainDish = order.OrderDetailPrinterModels.Where(p => p.ParentId == null); // ordersOfMainDish là một danh sách các món ăn chính ví dụ: trà sữa, cà phê
                    int count = 0;
                    foreach (var orderOfMainDish in ordersOfMainDish)
                    {
                        if (whitelistCode.Count > 0)
                        {
                            ...
                            if (whitelistCode.Any(q => q == orderOfMainDish.CatID))
                            {
                                for (int i = 0; i < orderOfMainDish.ItemQuantity; ++i)
                                {
                                    ...
                                    printDocumentInner.PrintPage += (sender, args) => CreateCookBillWithExtra(args.Graphics, 
                                        order, isPrintAllOrderDetail, orderOfMainDish.TmpDetailId ?? 0, orderOfMainDish.ItemQuantity);

                                    printDocumentInner.PrinterSettings.PrinterName = storeInfo.PrinterLabel;
        ...
        public void PrintBillMomo( OrderPrinterModel order, BillTypeEnum billType,
        ...
        public void PrintWifiPassword( OrderPrinterModel order = null)
        {
            try
            {
                var fileName = "";
                ...
                printDocument.PrinterSettings.PrinterName = storeInfo.PrinterBill;
                CustomPrintLocation(printDocument, fileName);
            }
            catch (Exception e)
            {
        ...
        public Image GetImageReport( OrderPrinterModel order)
        {
        ...
        //private void CreateVAT(object sender, PrintPageEventArgs e,  OrderPrinterModel order, CustomerInfoModel cusInfo)
        //{
        //    //Draw Invoice
        ...
        private int CreateReceipt(Graphics graphic,  OrderPrinterModel order)
        {
            ...
            // Address.
            if (storeInfo.TerminalAddress != null)
            {
                string[] splitAddress = storeInfo.TerminalAddress.Split('|');
            ...
            // Hotline.
            var hotline = string.Format("Hotline Delivery: {0}", storeInfo.HotLine);
            offset = WriteCenterText(graphic, offset + 7, fVerdana9B, hotline);

            // Title. 
            if (order.OrderType == (int)OrderTypeEnum.Delivery
                || order.PaymentPrinterModels.Any(a => a.Type == (int)PaymentTypeEnum.GrabPay || a.Type == (int)PaymentTypeEnum.GrabPay))
            {
            ...
            //var time = Utils.UtcDateTimeNow().ToString("dd/MM/yyyy  hh:mm:sstt");
            ...
            // ID.
            text = storeInfo.OrderCodeText + string.Format(":\t{0}", order.OrderCode);
            ...
            //{
            //    var tableViewModel
            //        = AllTables.LastOrDefault(t => t.Id == order.TableId);

            //    if (tableViewModel != null)
            //        order.TableNumber = tableViewModel.Number;
            //}

            var textOnline = storeInfo.OnlineOrderText;
            // Table.
            var textTakeAway = storeInfo.TakeAwayText + string.Format(": {0}",
                order.TableId == null ? " - " + textOnline : order.TableNumber);
            var textAtStore = storeInfo.AtStoreText + string.Format(":\t\t{0}",
                order.TableId == null ? " - " + textOnline : order.TableNumber);
            var textDelivery = storeInfo.DeliveryText;
            ...
            #region TTH
            foreach (OrderDetailPrinterModel orderDetailPrinterModel in sortedOrderDetailList)
            {
                if (!orderDetailPrinterModel.IsExtraProduct)
                {
                    {
                        string productName = orderDetailPrinterModel.ProductName;
                        int charPerLine = 27;
                        ...
                        if (orderDetailPrinterModel.Quantity > 99)
                        {
                            indentQuantity += 9;
                            indentUnitPrice += 7;
                            if (orderDetailPrinterModel.Quantity > 999)
                            {
                                indentQuantity += 9;
                                indentUnitPrice += 7;
                            }
                        ...
                        WriteLeftText(graphic, offset + 2, fCourier9B, orderDetailPrinterModel.Quantity.ToString(), 0);
                        WriteLeftText(graphic, offset + 2, fCourier9B, "x", indentQuantity);

                        // Price
                        WriteLeftText(graphic, offset + 2, fCourier9B, orderDetailPrinterModel.UnitPrice.ToString("N0"), indentUnitPrice);

                        // Total.
                        var total = orderDetailPrinterModel.TotalAmount;
                ...
                {
                    text = string.Format("{0}", "+ " + orderDetailPrinterModel.ProductName);
                    ...
                    // ProductViewModel amount price.
                    WriteLeftText(graphic, offset + 2, fCourier9B, orderDetailPrinterModel.Quantity.ToString(), 55);
                    WriteLeftText(graphic, offset + 2, fCourier9B, "x", 70);

                    // Price
                    WriteLeftText(graphic, offset + 2, fCourier9B, orderDetailPrinterModel.UnitPrice.ToString("N0"), 85);
                ...
                if (orderDetailPrinterModel.Discount > 0)
                {
                    // Discount.
                    text = string.Format("-{0}", orderDetailPrinterModel.Discount.ToString("N0"));
                    //text = "discount";
                    WriteCenterTextAlignRight(graphic, offset, fCourier9B, text);

                    // Payment.
                    offset = WriteRigth64Text(graphic, offset, fCourier9B, orderDetailPrinterModel.FinalAmount.ToString("N0"));
                }
            }
            ...
            // Promotion
            //var appliedPromotions = new List<PromotionPrinterModel>();
            //foreach (var mapping in order.OrderPromotionMappingPrinterModels)
            //{
            //    var applied = appliedPromotions.FirstOrDefault(m => m.PromotionCode == mapping.PromotionCode);
            //    if (applied == null)
            //    {

            //        var promotion = Promotions.FirstOrDefault(p => p.PromotionCode == mapping.PromotionCode);
            //        if (promotion != null)
            //        {
            ...
            //foreach (var od in order.OrderDetailPrinterModels)
            //{
            //    foreach (var mapping in od.OrderDetailPromotionMappingPrinterModel)
            //    {
            //        var applied = appliedPromotions.FirstOrDefault(m => m.PromotionCode == mapping.PromotionCode);
            //        if (applied == null)
            //        {
            ...
            //    }
            //}
            var appliedPromotions = order.AppliedPromotions;
            ...
            // FinalAmount.
            //Passio
            var orderVM = order.PaymentPrinterModels.FirstOrDefault();
            ...
            if (order.OrderType != (int)OrderTypeEnum.Delivery)
            {
                ...
                //Loại thanh toán
                var orderPayment = order.PaymentPrinterModels;
                if (order.PaymentPrinterModels.Any(p => p.Type != (int)PaymentTypeEnum.Cash
                                                        && p.Type != (int)PaymentTypeEnum.ExchangeCash))
                {
                    //Tiền mặt
                    var cash = order.PaymentPrinterModels
                    ...
                    //Member
                    cash = order.PaymentPrinterModels
                    ...
                    if (cash > 0)
                    {
                        ...
                        if (order.CurrentCustomerModel != null)
                        {
                            var accountCredit = order.CurrentCustomerModel
                ...
                var returnMoney = order.PaymentPrinterModels.Where(p => p.Amount < 0).Sum(p => p.Amount);
                ...
                // Wifi.
                if (storeInfo.IsShowWifiInfo.Trim().ToLower() == "true")
                {
                    text = "WIFI: " + storeInfo.WifiName;
                    offset = WriteLeftText(graphic, offset + 5, fVerdana8, text, 5);

                    if (storeInfo.IsGeneratePassWifi.Trim().ToLower() == "true")
                    {
                        var passWifi = "";
                        if (!order.HasOrder)
                        {
                            passWifi = ApiThirdParty.GenerateWifiPassForLocation(storeInfo.WiSkyLocationId.Trim(), null);
                        }
                        else
                        {
                            ...
                            {
                                passWifi = ApiThirdParty.GenerateWifiPassForLocation(storeInfo.WiSkyLocationId.Trim(), order.CheckInDate);
                    ...
                    {
                        text = "PASS: " + storeInfo.WifiPassword;
                    }
                    offset = WriteLeftText(graphic, offset + 5, fVerdana8, text, 5);
                }
            }
            ...
            // Thankyou.
            if (storeInfo.EndingTextOnBill != null)
            {
                var line = "---------------";
                offset = WriteCenterText(graphic, offset + 5, fVerdana8B, line);
                //offset = DrawDashLine(graphic, offset + 5);
                string[] splitEndText = storeInfo.EndingTextOnBill.Split('|');
            ...
            var momo = false;
            //if (storeInfo.MomoEnable.Trim().ToLower() == "true")
            //{
            //    var momoCash = order.PaymentPrinterModels
            //        .Where(p => p.Type == (int)PaymentTypeEnum.MoMo)
            //        .Sum(p => p.Amount);
            //    if (momoCash > 0)
            //    {       
            ...
            if (!momo)
            {
                if (storeInfo.QRCodeEnable.Trim().ToLower() == "true")
                {
                    text = storeInfo.QRCodeURL.ToString();
                    offset = DrawQRCode(graphic, offset + 15, text);

                    text = storeInfo.QRCodeDescription.ToString();
        ...
        private int CreateMomoReceipt(Graphics graphic,  OrderPrinterModel order)
        {
            var fVerdana9B = new Font("Verdana", 9, FontStyle.Bold);
            var fVerdana8 = new Font("Verdana", 8, FontStyle.Regular);
            var fVerdana6 = new Font("Verdana", 6, FontStyle.Regular);
            ...
            //if (storeInfo.SkyConnectEnable.Trim().ToLower() == "true")
            //{
            ...
            //    {
            //        if (storeInfo.ZaloEnable.Trim().ToLower() == "true")
            //        {
            //            offset = SkyConnectDrawQRCode(graphic, offset + 15, billId, amount, dateTime);
        ...
        private void CreateWifiPassword(Graphics graphic,  OrderPrinterModel order = null)
        {
            var fVerdana9B = new Font("Verdana", 9, FontStyle.Bold);
            var fVerdana8 = new Font("Verdana", 8, FontStyle.Regular);
            var fVerdana6 = new Font("Verdana", 6, FontStyle.Regular);
        ...
        private void CreateCookBill(Graphics graphic,  OrderPrinterModel order, bool isPrintAllOrderDetail)
        {
            int charPerLine = 27;
            ...
            var text = Utils.UtcDateTimeNow().ToString("dd/MM/yyyy HH:mm");
            ...
            List<OrderDetailPrinterModel> sortedOrderDetailList = new List<OrderDetailPrinterModel>();
            if (isPrintAllOrderDetail)
            {
                sortedOrderDetailList = GetSortedValidOrderDetail(order); //Get alls valid  OrderPrinterModel details to 
            }
            else
            {
        ...
        private void CreateCookBillDetail(Graphics graphic,  OrderPrinterModel order, OrderDetailViewModel orderDetail)
        {
            int charPerLine = 27;
        ...
        /// <param name="order"></param>
        /// <param name="isPrintAllOrderDetail"></param>
        private void CreateCookBillExtra(Graphics graphic, OrderPrinterModel order, bool isPrintAllOrderDetail, int? printGroup = null)
        {
            int charPerLine = 27;
        ...
        /// <param name="order"></param>
        /// <param name="isPrintAllOrderDetail"></param>
        private void CreateCookBillWithExtra(Graphics graphic,  OrderPrinterModel order, bool isPrintAllOrderDetail, int mainOrderId, int mainOrderItemQuantity)
        {
            int charPerLine = 27;
        ...
        //    // Address.
        //    offset = WriteCenterText(graphic, offset + 10, fVerdana9B, storeInfo.TerminalAddress == null ? "" : storeInfo.TerminalAddress.ToUpper());

        //    // Hotline.
        //    var hotline = string.Format("{0}", storeInfo.TerminalName);
        //    offset = WriteCenterText(graphic, offset + 7, fVerdana9B, hotline);
        ...
        //    // Time.
        //    //var time = Utils.UtcDateTimeNow().ToString("dd/MM/yyyy HH:mm");
        ...
        //            //- category.Discount;
        //            if (posConfig.IsShowMoneyReport.Trim().ToLower() == "true")
        //            {
        ...
        //                    ProductViewModel tmpProduct = AllProducts.FirstOrDefault(p => p.Code == code);
        ...
        //    // Date.
        //    text = Utils.UtcDateTimeNow().ToString("dd/MM/yyyy HH:mm");
        ...
        private int CreateReceiptPaymentMember(Graphics graphic, string cardCode, string cardName, double currentAmount, double paymentAmount,string accountId=null)
        {
            var fVerdana9B = new Font("Verdana", 9, FontStyle.Bold);
            var fVerdana8 = new Font("Verdana", 8, FontStyle.Regular);
            var fVerdana6 = new Font("Verdana", 6, FontStyle.Regular);
            ...
            var text = string.Format("NHÂN VIÊN:\t{0}", accountId);
            offset = WriteLeftText(graphic, offset + 7, fVerdana8, text);
        ...
        private void CreateOpenSafe(Graphics graphic, string name, string reason)
        {
            ...
            text = ResManager.GetString("Printer_OpenSafe_Title", CultureInfo);
            offset = WriteCenterText(graphic, offset + 5, fVerdana10B, text);
            offset += (int)fontItem.GetHeight();

            text = string.Format(ResManager.GetString("Printer_Username_Opensafe", CultureInfo) + ":\t{0}", name);
            offset = WriteCenterText(graphic, offset + 5, fVerdana8, text);
            offset += (int)fontItem.GetHeight();

            text = string.Format(ResManager.GetString("Printer_Reason_Opensafe", CultureInfo) + ":\t{0}", reason);
            offset = WriteCenterText(graphic, offset + 5, fVerdana8, text);
            offset += (int)fontItem.GetHeight();
        }
        ...
        /// <summary>
        /// Get all valid OrderDetailViewModel and sort by  OrderPrinterModel : mainitem -> extraitem of mainitem
        /// </summary>
        /// <param name="order"></param>
        /// <returns></returns>
        private List<OrderDetailPrinterModel> GetSortedValidOrderDetail(OrderPrinterModel order)
        {
            var validOrderDetils = order.OrderDetailPrinterModels.Where(od => (od.Status != (int)OrderDetailStatusEnum.Cancel
            ...
            var sortedOrderDetailList = new List<OrderDetailPrinterModel>();
        ...
        private List<OrderDetailPrinterModel> GetSortedNewOrderDetail( OrderPrinterModel order)
        {
            var validOrderDetils = order.OrderDetailPrinterModels.Where(od => (od.Status == (int)OrderDetailStatusEnum.New));
        ...
        private int DrawLogo(Graphics graphic, int offset)
        {
            ...
            var logo = Utils.GetImage(storeInfo.LogoPrint);
                                            ...
                                            DateTime time)
        {
            ...
            var size = 100;             //QR size nhỏ
            var ix = (250 - size) / 2;  //Canh giữa cho QR
            var isSkyConnectEnable = bool.Parse(storeInfo.SkyConnectEnable.Trim().ToLower());
        ...
        /// <summary>
        /// Function này hốt từ function PrintBills ở file SaleScreen3.cs qua
        /// </summary>
        /// <param name="order"></param>
        public static void PrintBillsVersion2(OrderPrinterModel order)
        {
            Printer printer = new Printer();
            if (!string.IsNullOrEmpty(storeInfo.PrintBillOrder))
            {
                var bills = storeInfo.PrintBillOrder.Split('|');
                foreach (var bill in bills)
                {
                    var printerName = storeInfo.PrinterBill;
                    var type = BillTypeEnum.Customer;
                    if (bill.Contains("cook"))
                    {
                        printerName = storeInfo.PrinterCook1;
                        type = BillTypeEnum.Cook;
                        printer.PrintBill(order, type, printerName, true);
                        if (posConfig.SaveTableStatus.ToLower() != "true")
                        {
                            var tmpOrders = order.OrderDetailPrinterModels;
                            if (storeInfo.PrinterDetail.ToLower() == "true")
                            {
                                printerName = storeInfo.PrinterLabel;
                                type = BillTypeEnum.Extra;
                                if (order.PaymentPrinterModels.Any(pev => pev.Type == (int)PaymentTypeEnum.MoMo))
                                {
                                    printer.PrintBillMomo(order, type, printerName, true);
                                }
                                else
                                {
                                    printer.PrintBill(order, type, printerName, true);
